...
class FlattenStructure(private val rootPackage: String?) : ScanningRecipe<AtomicReference<Pair<Int, String?>>>() {
    ...
    override fun getInitialValue(ctx: ExecutionContext) = AtomicReference<Pair<Int, String?>>(0 to null)

    override fun getScanner(acc: AtomicReference<Pair<Int, String?>>): TreeVisitor<*, ExecutionContext> {
        ...
        if (currentPackage?.second == "") return TreeVisitor.noop<Tree, ExecutionContext>()
        return object : KotlinIsoVisitor<ExecutionContext>() {
            override fun visitCompilationUnit(cu: K.CompilationUnit, ctx: ExecutionContext): K.CompilationUnit {
                ...
                val currentPackage = acc.get()?.second
                val visitCounts = acc.get().first + 1
                println("FlattenStructure: $visitCounts files visited")
                // First scanned file
                if (currentPackage == null) acc.set(visitCounts to packageName)
                else {
                    ...
                    acc.set(visitCounts to commonPrefix)
                }
                return cu
            }
        }
    ...
    override fun getVisitor(acc: AtomicReference<Pair<Int, String?>>): TreeVisitor<*, ExecutionContext> {
        return object : KotlinIsoVisitor<ExecutionContext>() {
            override fun visitCompilationUnit(cu: K.CompilationUnit, ctx: ExecutionContext): K.CompilationUnit {
                val packageName = cu.packageDeclaration?.packageName ?: return cu
                val packageToSet: String? = rootPackage ?: acc.get()?.second
